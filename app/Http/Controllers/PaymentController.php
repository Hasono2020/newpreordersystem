...
class PaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($tripId) {
            ...
            // ── Recompute COMBINED balance per customer (shipping charged once on total weight) ──
            // Only the ~50 rows on the current page are processed, so this stays fast at scale.
            $custIds = collect($outstanding->items())->pluck('customer_id')->all();
            if (!empty($custIds)) {
                // Pull all active items for these customers' orders in this trip, with product weight
                $itemRows = DB::table('order_items')
                    ->join('orders', 'orders.id', '=', 'order_items.order_id')
                    ->join('products', 'products.id', '=', 'order_items.product_id')
                    ->where('orders.trip_id', $tid)
                    ->whereIn('orders.customer_id', $custIds)
                    ->whereNotIn('order_items.status', ['cancelled', 'sold_out'])
                    ->select('orders.customer_id', 'orders.shipping_area_id',
                             'products.weight_gram', 'order_items.quantity')
                    ->get();

                // Sum combined weight per customer + capture a shipping area id
                $weightByCust = [];
                $areaByCust   = [];
                foreach ($itemRows as $r) {
                    $weightByCust[$r->customer_id] = ($weightByCust[$r->customer_id] ?? 0)
                        + ((int)$r->weight_gram * (int)$r->quantity);
                    if (!isset($areaByCust[$r->customer_id]) && $r->shipping_area_id) {
                        $areaByCust[$r->customer_id] = $r->shipping_area_id;
                    }
                }

                $areas = \App\Models\ShippingArea::whereIn('id', array_filter(array_values($areaByCust)))
                    ->get()->keyBy('id');

                // Pre-load per-order shipping sums for all page customers in ONE query
                $shipSums = DB::table('orders')
                    ->where('trip_id', $tid)
                    ->whereIn('customer_id', $custIds)
                    ->where('payment_status', '!=', 'paid')
                    ->groupBy('customer_id')
                    ->pluck(DB::raw('SUM(shipping_fee)'), 'customer_id')
                    ->all();

                foreach ($outstanding->items() as $row) {
                    $cid       = $row->customer_id;
                    $weight    = $weightByCust[$cid] ?? 0;
                    $areaId    = $areaByCust[$cid] ?? null;
                    $area      = $areaId ? ($areas[$areaId] ?? null) : null;
                    $combinedShipping    = $area ? $area->calcShippingFee($weight) : 0;
                    $sumPerOrderShipping = (float)($shipSums[$cid] ?? 0);

                    // Subtract the per-order shipping already baked into SUM(total_amount),
                    // then add the single combined shipping figure.
                    $row->total_ordered = $row->total_ordered - $sumPerOrderShipping + $combinedShipping;
                    $row->balance_due   = $row->total_ordered - $row->total_paid;
                    $row->combined_shipping      = $combinedShipping;
                    $row->per_order_shipping_sum = $sumPerOrderShipping;
                }
            }
        }
